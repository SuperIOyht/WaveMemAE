...
class WavePool_Two(nn.Module):
    def __init__(self, in_channels):
        super(WavePool_Two, self).__init__()
...
class WavePool_Two_Upscale(nn.Module):
    def __init__(self, in_channels):
        super(WavePool_Two_Upscale, self).__init__()
        self.LL, self.LH, self.HL, self.HH = get_wav_two(in_channels)
        self.up = nn.UpsamplingBilinear2d(scale_factor=2)
    def forward(self, x):
        return self.up(self.LL(x)), self.up(self.LH(x)), self.up(self.HL(x)), self.up(self.HH(x))
